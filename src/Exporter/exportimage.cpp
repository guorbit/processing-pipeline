...
ExportImage::ExportImage(unsigned char *img, int w, int h, int chan, std::string path) : image(img), width(w), height(h), channels(chan), filepath(path)
...
{ // Destructor to destroy declared variables

    stbi_image_free(image);
    std::cout << "Memory has been freed" << std::endl;
}

void ExportImage::SaveImage()
